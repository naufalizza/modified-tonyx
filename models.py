...
class MMRegressor(nn.Module):
    ...
    def __init__(self, model_path):
        ...
        if "distilbert" in model_path:
            ...
            self.reg_model = DistilBertModel.from_pretrained(model_path)
        else:
...
class Reg_Trainer():
    ...
    def dataset(self, pickled_compiled_data_path):
        compiled_data = []
        with open(pickled_compiled_data_path, "rb") as f:
            _raw_compiled_data = pickle.load(f)
            for raw_compiled_item in _raw_compiled_data:
                _compiled_item = {}
                _text1 = "\n".join(list(raw_compiled_item[1]))
                _text2 = "\n".join(list(raw_compiled_item[2]))
                _scores = raw_compiled_item[3]
                _compiled_item["text1"] = _text1
                _compiled_item["text2"] = _text2

                _compiled_item["Geography"] = _scores[0]
                _compiled_item["Entities"] = _scores[1]
                _compiled_item["Time"] = _scores[2]
                _compiled_item["Narrative"] = _scores[3]
                _compiled_item["Overall"] = _scores[4]
                _compiled_item["Style"] = _scores[5]
                _compiled_item["Tone"] = _scores[6]
                compiled_data.append(_compiled_item)
    ...
    def dataset_original_method(self, data_path):
